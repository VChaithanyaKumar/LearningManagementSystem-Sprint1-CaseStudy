...
namespace LMS.DAL
{
    public class LearnerRepository:ILearnerRepository
    {
        ...
        /*public DateTime GetSubmitDate(User user, string CourseTitle)
        {
            try
            {
                command = new SqlCommand("GetSubmitDate", connection)
                {
                    CommandType = CommandType.StoredProcedure
                };
                command.Parameters.AddWithValue("@UserEmail", user.UserEmail);
                ...
                if (dataReader.HasRows)
                {
                    ...
                    *//*DateTime date = new DateTime();
                    date = 0;*//*
                    DateTime date = (DateTime)dataReader["DateofCompletion"];
                    return date;
                }
                return;
